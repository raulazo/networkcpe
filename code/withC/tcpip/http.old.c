// this is a special version where the html escape codes are 2 characters
...
///
/// HTTP_USE_CHUNKS - Set to TRUE if the HTTP server will be 1.1 compliant
///      and will use Transfer-Encoding: chunked.  Default is FALSE.
///      Note: this may not be working well, so leave it to FALSE.
///
...
///    the file doesn't exist, will save 0 to retAddress.
///
...
/// * Author               Date        Comment
/// *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// * Darren Rook          7/14/06     Fixed a bug that prevented pages larger
///                                    than max packet size from being sent.
///
//////////////////////////////////////////////////////////////////////////////
...
char TCPPutFileParseConst(int32 *retAddy, int16 *n, int8 doSend)
{
   ...
   {
      read_program_memory(addy++, &stopC, 1);
      while (stopC && (stopC!='%') && (fileSize < max))
      {
         if (doSend)
            tcp_http_putc(stopC);
         read_program_memory(addy++, &stopC, 1);
   ...
   for (hs=0; hs<HTTP_NUM_SOCKETS; hs++)
   {
      ...
      switch(http_state[hs])
      {
         ...
         case HTTP_GET_POST_CONTINUE:
            while (TCPIsGetReady(currSocket) && (http_post_len[hs] != 0))
            {
               TCPGet(currSocket, &c);
